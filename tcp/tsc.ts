...
import { HTTPReq, TCPConn, DynBuf, BodyReader } from "./types";
import { HTTPError } from "./http-error";
import { cutMessage } from "./http-parser";
import { Socket } from "dgram";
import { bufpush, bufPop } from "./buffer-utils";

/**
 * HTTP Response Interface
 *
 * Represents the structure of an HTTP response that will be sent back to the client.
 *
 * @interface HTTPRes
 * @property {number} code - HTTP status code (e.g., 200, 404, 500)
 * @property {Buffer[]} headers - Array of HTTP headers as Buffer objects
 * @property {BodyReader} body - Body content reader for streaming response data
 */
interface HTTPRes {
  code: number;
  headers: Buffer[];
...
function encodeHTTPResp(resp: HTTPRes): Buffer {
  // Build response as string first for clarity
  let response = `HTTP/1.1 ${resp.code} OK\r\n`;
  ...
  for (const header of resp.headers) {
    if (header.toString().toLowerCase().startsWith("content-length:")) {
  ...
  if (!hasContentLength) {
    response += `Content-Length: ${resp.body.length}\r\n`;
  }

  // Add empty line to separate headers from body (CRITICAL!)
  response += "\r\n";

  console.log("Response headers being sent:");
  console.log(response.replace(/\r/g, "\\r").replace(/\n/g, "\\n"));
  console.log("Response should end with \\r\\n\\r\\n before body");
  ...
  return {
    ...
    read: async (): Promise<Buffer> => {
      ...
      if (buf.length === 0) {
        ...
        bufpush(buf, data); // Add new data to buffer
      ...
      bufPop(buf, consume); // Remove consumed data from buffer
 ...
 */
// Also, make sure your handleReq is clean:
async function handleReq(req: HTTPReq, body: BodyReader): Promise<HTTPRes> {
  console.log(`=== Handling ${req.method} ${req.uri.toString()} ===`);
  ...
  console.log("Response created:", {
    code: response.code,
    headers: response.headers.map((h) => h.toString()),
...
async function writeHTTPResp(conn: TCPConn, resp: HTTPRes): Promise<void> {
  console.log("=== Writing HTTP Response ===");

  if (resp.body.length < 0) {
    throw new Error("TODO: chunked encoding");
  }

  // Don't modify the original headers array
  const originalHeaderCount = resp.headers.length;
  console.log(
    "Original headers:",
    resp.headers.map((h) => h.toString())
  );

  // Write headers
  const headerBuffer = encodeHTTPResp(resp);
  console.log("Header buffer size:", headerBuffer.length);

  await soWrite(conn, headerBuffer);
  ...
  while (true) {
    ...
    totalBodyBytes += data.length;
    console.log(`Writing body chunk: ${data.length} bytes`);
    await soWrite(conn, data);
  }

  console.log(`Total body bytes written: ${totalBodyBytes}`);
  console.log("=============================");
}
